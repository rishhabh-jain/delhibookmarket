...
import { Plus, ShoppingCart, Loader2, Tag, Check, X, Edit3 } from "lucide-react"
...
export const ComboProduct: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    if (fullProducts.length > 0) {
      const names = fullProducts.map(p => p.name).join(" + ")
  ...
  const handleAddComboToCart = async () => {
    ...
    if (onAddToCart && fullProducts.length > 0) {
      onAddToCart(fullProducts)
    } else {
      // Use the new addCombo method
      const wooProducts = fullProducts.map(fullProductToWooProduct)
  ...
  const handleRemoveProductFromCombo = (productId: number) => {
    if (comboId) {
      ...
      const updatedProducts = fullProducts.filter(p => p.id !== productId)
      setFullProducts(updatedProducts)
      
      // If no products left, reset the combo state
          ...
          {fullProducts.map((product, index) => (
            <React.Fragment key={index}>
              <div className="flex items-center gap-3 p-2 rounded-md bg-gray-50 hover:bg-gray-100 transition-colors group">
                ...
                {/* Show remove button if combo is in cart */}
                {isAddedToCart && comboId && (
                  <motion.button
                ...
                {index < fullProducts.length - 1 && !isAddedToCart && (
            ...
            <div className="space-y-2">
              ...
              <details className="group">
                ...
                <div className="mt-2 pt-2 border-t border-gray-100 space-y-2">
                  <p className="text-xs text-gray-600 mb-2">Add more books to this combo:</p>
                  {/* You can add a product search/selection interface here */}
                  <div className="text-xs text-gray-500 italic">
                    Product selection interface can be added here
                  </div>
                </div>
              </details>
            </div>
